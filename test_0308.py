...
for i in range(10000):
    if vol1<60*35*0.4*por[0]:     #1档料球体体积占关心区域体积比
        pass
    else:
        break
...
for i in trange(len(tri.simplices)):
    point1,point2,point3=tri.simplices[i][0],tri.simplices[i][1],tri.simplices[i][2]        #该四面体所包含的球心点编号，共4个，认为前3个点构成底面，最后一个点为顶点
    radius1,radius2,radius3=ball_dic[point1+1][1][0],ball_dic[point2+1][1][0],ball_dic[point3+1][1][0]       #第1个球的半径

    x=ball_dic[point1+1][0][0]+(ball_dic[point2+1][0][0]-ball_dic[point1+1][0][0])*radius1/(radius1+radius2)   #第1个顶点的x坐标
    y=ball_dic[point1+1][0][1]+(ball_dic[point2+1][0][1]-ball_dic[point1+1][0][1])*radius1/(radius1+radius2)
    Vertexs.append([x,y])

    x=ball_dic[point1+1][0][0]+(ball_dic[point3+1][0][0]-ball_dic[point1+1][0][0])*radius1/(radius1+radius3)  #第2个顶点的x坐标
    y=ball_dic[point1+1][0][1]+(ball_dic[point3+1][0][1]-ball_dic[point1+1][0][1])*radius1/(radius1+radius3)
    Vertexs.append([x,y])

    x=ball_dic[point2+1][0][0]+(ball_dic[point3+1][0][0]-ball_dic[point2+1][0][0])*radius2/(radius2+radius3)  #第4个顶点的x坐标
    y=ball_dic[point2+1][0][1]+(ball_dic[point3+1][0][1]-ball_dic[point2+1][0][1])*radius2/(radius2+radius3)
    Vertexs.append([x,y])
