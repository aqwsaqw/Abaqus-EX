...
Mesh_File_path = 'E:\\temp\\260106/'
Model_File_path = 'E:\\temp\\260106/'
...
for i in tqdm(mat_ele):  # 首先给集料内接球内的单元赋予属性
    for j in sto_dic.keys():  # 遍历集料编号
        ...
        y = Element_core[i][1]
        z = Element_core[i][2]
        if (x - x0) ** 2 + (y - y0) ** 2/0.8/0.8 + (z - z0) ** 2/0.6/0.6  <= rad_min ** 2: #TODO
            sto_ele.append(i)
            break
        else:
            pass
...
for i in tqdm(mat_ele):  # 判断该单元是否位于集料外接球外，若在外面直接跳过该集料
    for j in sto_dic.keys():  # 遍历集料编号
        ...
        y = Element_core[i][1]
        z = Element_core[i][2]
        if (x - x0) ** 2 + (y - y0) ** 2 / 0.8/0.8 + (z - z0) ** 2/0.6/0.6 > rad_max ** 2:
            pass
        else:
            ...
            if ConvexHull(vertex_temp2) == ConvexHull(vertex_temp1):
                ...
                pass
asp_ele = cem_ele + mat_ele#sorted(set(mat_ele) - set(sto_ele)) #排除石头的基体单元的标签(沥青)+mat中残余沥青
print(len(asp_ele))
...
ASnum1 = max(Element_dic.keys())+1  # AA单元中编号最大的单元编号
...
'''
# (17-5)插入CS单元 CS->AA
AA2num1 = max(Element_dic.keys()) + 1
for i in CSloc_dic.keys():
    ...
    Element_dic.update({i: [NODE1, NODE2, NODE3, NODE4, NODE5, NODE6, NODE7, NODE8]})
AA2num2 = max(Element_dic.keys())  # AS单元中编号最大的单元编号
AA2_SET = range(AA2num1, AA2num2 + 1)  # 列表，里面是所有AS单元的编号
'''
...
print(CO_TYPE, file=outfile)
for EL in range(TEle_Value1, AAnum2-1):
    #print(EL)
...
for EL in range(ASnum1-1, TEle_Value2):
    #print(EL)
    print("%5d, %5d, %5d, %5d, %5d, %5d, %5d, %5d, %5d" % (
    EL + 1, Element_dic[EL + 1][0], Element_dic[EL + 1][1], Element_dic[EL + 1][2], \
    Element_dic[EL + 1][3], Element_dic[EL + 1][4], Element_dic[EL + 1][5], Element_dic[EL + 1][6],
...
grout_rate = len(cem_ele) / (60 * 35 * 30 * (void_rate + 0.1))
print('模型灌浆率为：', grout_rate, len(asp_ele)/(60*35*30), len(sto_ele)/(60*35*30))
